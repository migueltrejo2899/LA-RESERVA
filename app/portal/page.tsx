...
export default async function PortalOrders() {
  ...
  return (
    <div className="card">
      <h3 className="font-display text-lg mb-4">Tus pedidos</h3>
      {(!orders || orders.length === 0) && <p className="text-inksoft text-sm">Aún no tienes pedidos registrados.</p>}
      <div className="divide-y divide-line">
        {orders?.map((o: any) => {
          const paid = (o.payments || []).reduce((s: number, p: any) => s + Number(p.monto), 0)
          return (
            <Link key={o.id} href={`/portal/pedidos/${o.id}`} className="flex justify-between items-center py-4 flex-wrap gap-3 hover:bg-crate/5">
              <div>
                <div className="font-mono text-xs text-inksoft">{o.folio} · {fmtDate(o.created_at)}</div>
                <div className="font-semibold font-mono">{fmtMoney(o.total)}</div>
              </div>

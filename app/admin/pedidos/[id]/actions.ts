...
export async function updateOrder(formData: FormData) {
  ...
  if (!clientId) {
    redirect(`/admin/pedidos/${orderId}?error=${encodeURIComponent('Selecciona un cliente.')}`)
  }
  if (!folio) {
    redirect(`/admin/pedidos/${orderId}?error=${encodeURIComponent('El folio no puede quedar vacío.')}`)
  }

  await supabase.from('orders').update({ client_id: clientId, folio }).eq('id', orderId)
...
export async function updateOrderDate(formData: FormData) {
  const supabase = createClient()
  const orderId = String(formData.get('orderId') || '')
  const fecha = String(formData.get('fecha') || '')

  if (!fecha) {
    redirect(`/admin/pedidos/${orderId}?error=${encodeURIComponent('Selecciona una fecha válida.')}`)
  }

  await supabase.from('orders').update({ created_at: fecha }).eq('id', orderId)
